...
export const updateRecipe = async ({ recipeId, token, formData }: Props): Promise<IResponse> => {
    ...
    try {
        ...
        await valeryClient(`/production/recipes/${recipeId}`, {
            method: "PATCH",
            headers: {
        ...
        return {
            error: false,
            message: "Se actualizó la receta con éxito."
        }
    } catch (error) {
        if (isApiError(error)) {
            return {
